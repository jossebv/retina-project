...
#define TIM_AS_PWM2_MASK BASE_MASK_TO_POS(0x0060,8)
...
static void _timer_pwm_setup(uint32_t tx_id)
{
  ...
  uint32_t psc = ceil(((SystemCoreClock/(2 * NEC_PWM_FREQ_HZ)) / 65536)) - 1;
  TIM2->PSC = psc;

  /*Set the value of the autoreload register*/
  uint64_t arr = (SystemCoreClock/(2*NEC_PWM_FREQ_HZ))/(TIM2->PSC+1) - 1;
  ...
  /*Set both mode PWM 1 and enable preload register of CH2*/
  TIM2->CCMR1 |= TIM_AS_PWM2_MASK;
  TIM2->CCMR1 |= TIM_CCMR1_OC2PE;

  /*Set the pulse width of CH2*/
  uint32_t ccr2 = NEC_PWM_DC * (TIM2->ARR+1);
  TIM2->CCR2 = ccr2;  
}
